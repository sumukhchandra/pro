...
export default function NovelsPage() {
  ...
  const fetchBooks = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/api/content?type=novel&sortBy=${sortBy}&order=${sortOrder}`
      );
  ...
  const fetchMostLiked = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/content/most-liked?type=novel');
      const data = await response.json();
      setMostLiked(data);
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-dark-950 flex items-center justify-center">
        <div className="animate-pulse text-golden-500 text-xl">Loading novels...</div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {books.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-dark-300 mb-2">No novels found</h3>
