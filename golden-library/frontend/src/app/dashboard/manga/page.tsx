...
export default function MangaPage() {
  ...
  const fetchBooks = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/api/content?type=manga&sortBy=${sortBy}&order=${sortOrder}`
      );
  ...
  const fetchMostLiked = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/content/most-liked?type=manga');
      const data = await response.json();
      setMostLiked(data);
    } catch (error) {
      console.error('Error fetching most liked manga:', error);
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-dark-950 flex items-center justify-center">
        <div className="animate-pulse text-golden-500 text-xl">Loading manga...</div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {books.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-dark-300 mb-2">No manga found</h3>
